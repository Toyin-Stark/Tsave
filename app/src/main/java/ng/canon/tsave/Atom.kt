...
class Atom : AppCompatActivity() {
    ...
    fun mrSave(urld: String){
        ...
        val filename = topic
        val name = "twitch_$timeStamp" + "." + extension
        val dex = File(Environment.getExternalStorageDirectory().absolutePath, "tload")
        if (!dex.exists())
            dex.mkdirs()
        ...
        request.setDestinationInExternalPublicDir("/tload",  name)
